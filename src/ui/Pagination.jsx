...
const Pagination = ({
  ...
  const prev = (
    <li className={pageClasses} onClick={() => changeFunction(currentPage - 1)}>
      «
    </li>
  );
  const next = (
  ...
  const prevPage = hideArrows
    ? currentPage !== 1 && prev
    : currentPage !== 1
    ? prev
    : prevDisabled;
  const nextPage = hideArrows
    ? currentPage !== pagesNumber && next
      ...
      <ul className={listClasses}>
        ...
        {modifiedPages.map((page) => (
          <li
            key={page}
            className={
              currentPage === page ? pageClasses + " active" : pageClasses
            }
