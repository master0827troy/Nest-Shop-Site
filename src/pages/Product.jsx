...
import ProductDiscount from "../components/Products/Product/ProductDetails/ProductDiscount";
  ...
  useEffect(() => {
    if (reviews && usersData) {
      ...
      setReviewForm(
        isAuthenticated &&
          !reviews?.find(
            (review) => review?.userId === userId && review?.productId === id
          )
      );
    }
      ...
      <div className="flex flex-col xl:flex-row justify-between gap-12 lg:gap-14">
        <div className="grow">
          <div className="mb-12 flex flex-col md:flex-row gap-8">
            <div className="relative md:w-80 h-full shrink-0">
              {productData.images ? (
