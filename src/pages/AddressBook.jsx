...
const AddressBook = () => {
  ...
  const deleteAddress = async (address) => {
    try {
      const newAddresses = addresses.filter(
        (ele) => addresses.indexOf(ele) !== addresses.indexOf(address)
      );
  ...
  const deletePhone = async (phone) => {
    try {
      const newPhoneNumber = phoneNumbers.filter(
        (ele) => phoneNumbers.indexOf(ele) !== phoneNumbers.indexOf(phone)
      );
      await updateDoc(doc(db, "users", auth.currentUser.uid), {
    ...
    <div className="flex flex-col lg:flex-row gap-8">
      <div className="w-full lg:w-3/5 h-fit p-5 bg-gray-100 shadow-lg">
          ...
          {addresses.length !== 0 ? (
            addresses.map((address) =>
              editingAddress !== addresses.indexOf(address) ? (
                ...
                >
                  <div className="flex flex-row gap-2">
                    ...
                    <p className="font-semibold mr-4">
                      ...
                      {address.country}, {address.postalNumber}
                    </p>
                  </div>
                  <div className="flex flex-row gap-2">
                    <FaPen
                      ...
                      onClick={() =>
                        setEditingAddress(addresses.indexOf(address))
                      }
                    />
                    <FiTrash2
                      className="text-lg cursor-pointer transition duration-300 hover:text-orange-600 hover:scale-125"
                ...
                <AddressForm
                  key={addresses.indexOf(address)}
                  show={editingAddress !== null}
                  changeHandler={setEditingAddress}
                  callbackFunction={reFetchUserData}
        ...
        {addressForm && (
          <AddressForm
            show={addressForm}
            changeHandler={setAddressForm}
            callbackFunction={reFetchUserData}
            userId={auth.currentUser.uid}
            addresses={addresses}
          />
        )}
        {!addressForm && editingAddress === null && (
          <Button
            bg
            text="Add"
            className="h-fit"
      ...
      <div className="w-full lg:w-2/5 h-fit p-5 bg-gray-100 shadow-lg">
          ...
          {phoneNumbers.length !== 0 ? (
            phoneNumbers.map((phoneNumber) =>
              editingPhone !== phoneNumbers.indexOf(phoneNumber) ? (
                <div
                  key={phoneNumbers.indexOf(phoneNumber)}
                ...
                >
                  ...
                  <FaPen
                    className="text-lg cursor-pointer transition duration-300 hover:text-orange-600 hover:scale-125"
                    onClick={() =>
                      setEditingPhone(phoneNumbers.indexOf(phoneNumber))
                    }
                  />
                  <FiTrash2
                    className="text-lg cursor-pointer transition duration-300 hover:text-orange-600 hover:scale-125"
                    onClick={() => deletePhone(phoneNumber)}
                  />
                </div>
              ) : (
                <PhoneForm
                  key={phoneNumbers.indexOf(phoneNumber)}
                  show={editingPhone !== null}
                  changeHandler={setEditingPhone}
                  callbackFunction={reFetchUserData}
                  userId={auth.currentUser.uid}
        ...
        {phoneForm && (
          <PhoneForm
            show={phoneForm}
            changeHandler={setPhoneForm}
            callbackFunction={reFetchUserData}
            userId={auth.currentUser.uid}
            phoneNumbers={phoneNumbers}
          />
        )}
        {!phoneForm && editingPhone === null && (
          <Button
            bg
            text="Add"
            className="h-fit"
