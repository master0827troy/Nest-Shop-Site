  ...
  return (
    <div className="shadow-md w-fit h-fit p-5">
      ...
      {props.ratings.map((rating) => (
        <div
          key={rating.value}
          className="flex flex-row items-center justify-start gap-2 h-fit"
        >
          <span className="w-3">{rating.value}</span>
          <FaStar className="text-orange-600" />
          <div className="w-44 h-4 relative bg-gray-300">
            <div
              className="absolute h-full z-10 bg-orange-600"
              style={{
                width: `${(rating.rating / props.productReviews) * 100}%`,
              }}
            ></div>
          </div>
